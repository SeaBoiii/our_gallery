...
type CameraSession = { count: 1 | 4; target: number | null; aspectRatio: number }
...
export default function PolaroidPage() {
  ...
  const startCameraSession = (target: number | null) => {
    const rect = layout.photoRects[target ?? 0]
    setCameraSession({ count: target === null && required > 1 ? 4 : 1, target, aspectRatio: rect.width / rect.height })
  }
  ...
  return <main className="polaroid-page">
    ...
    {cameraSession ? <CameraCapture shotCount={cameraSession.count} aspectRatio={cameraSession.aspectRatio} onClose={() => setCameraSession(null)} onComplete={files => { const target = cameraSession.target; setCameraSession(null); void openPhotos(files, target) }} /> : null}
